...
vis = visdom.Visdom(env='ZSL-base')
...
def valid(val_loader, val_cls_list, val_attrs, model_dir):
    ...
    checkpoint = load_checkpoint(osp.join(model_dir, 'checkpoint_bi_1601.pth.tar'))
...
def main(args):
    ...
    for epoch in range(0, args.epochs):
        ...
        for i,d in enumerate(train_loader):
            ...
            mse_loss = get_mse_loss(outputs, attr_targets, args.batch_size)
        ...
        save_checkpoint({
            ...
            'epoch': epoch + 1,
            'best_top1': 0,
        }, False, fpath=osp.join(args.model_dir, 'checkpoint_bi_1601.pth.tar'))
        nu = valid(val_loader, val_cls_list, val_attrs, args.model_dir)
        save_checkpoint({
            'state_dict': model.module.state_dict(),
            'epoch': epoch + 1,
            'best_top1': 0,
        }, False, fpath=osp.join(args.model_dir, 'checkpoint_bi1601_{}.pth.tar'.format(nu)))
